...
exports.applyToJob = async (req, res) => {
  try {
    ...
    const application = await Application.create({ job: req.params.jobId, applicant: req.user._id,
        resume: req.user.resume, //assuming is stored in user profile
     });
...
exports.getApplicationById = async (req, res) => {
  try {
    ...
    if (!isOwner) return res.status(403).json({ message: "User not authorized to view this application" });
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    if (!app || app.job.company.toString() !== req.user._id.toString()) return res.status(403).json({ message: "User not authorized to update this application" }) 
