...
exports.getEmployerAnalytics = async (req, res) => {
    try {
        if (req.user.role !== "employer") return res.status(403).json({ message: "Only employers can get analytics" });
        ...
        const activeJobsLast7 = await Job.countDocuments({ company: companyId, createdAt: { $gte: last7Days, $lt: now } });
        ...
        const applicationsLast7 = await Application.countDocuments({ job: { $in: jobIds }, createdAt: { $gte: last7Days, $lt: now } });

        const applicationsPrev7 = await Application.countDocuments({ job: { $in: jobIds }, createdAt: { $gte: prev7Days, $lt: last7Days } });

        const applicantTrnd = getTrend(applicationsLast7, applicationsPrev7);

        //Hired Applicants trend
        const hiredLast7 = await Application.countDocuments({ job: { $in: jobIds }, status: "Accepted", createdAt: { $gte: last7Days, $lt: now } });
        ...
        const recentJobs = await Job.find({
            company: companyId,
        }).sort({ createdAt: -1 }).limit(5).select('title location type createdAt isClosed');
